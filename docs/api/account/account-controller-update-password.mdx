---
title: PUT /v1/account/password
description: Update password.
---

**Tag:** `Account` &nbsp;·&nbsp; **Operation:** `AccountController_updatePassword`

## Purpose

Lets a signed-in member rotate their password from inside the product. The member must supply their current password as well as the new one, so a stolen but unlocked device cannot quietly change the credential and lock the real owner out. On success the new password takes effect immediately for any future sign-in.

## Sequence

```mermaid
sequenceDiagram
  participant C as ChangePasswordScreen
  participant API as Backend
  participant DB as MongoDB
  C->>API: PUT /v1/account/password
  API->>DB: query
  DB-->>API: result
  API-->>C: 200
```

## Parameters

| Name | In | Required | Type | Description |
| ---- | -- | -------- | ---- | ----------- |
| `Authorization` | header | yes | `string` | Bearer accessToken |
| `Content-Type` | header | no | `string` | application/json |

## Request body

Schema: [`UpdatePasswordDto`](/data-model/update-password-dto)

| Property | Type | Required | Description |
| -------- | ---- | -------- | ----------- |
| `currentPassword` | `string` | yes |  |
| `newPassword` | `string` | yes |  |

## Responses

- **200** — User password succesfully updated — [`NewPairOfTokensDto`](/data-model/new-pair-of-tokens-dto)
- **400** — Some inputs are missed or wrong
- **401** — Invalid credentials
- **422** — Some inputs are invalid
- **429** — Too many requests, rate limit exceeded
- **500** — Error not handled

## Used by

- [`ChangePasswordScreen`](/mobile/screens/change-password-screen)
